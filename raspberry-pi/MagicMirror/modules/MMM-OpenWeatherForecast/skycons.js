...
(function (global) {
   ...
   * GREAT JUSTICE.
   */
  let cancelInterval; let requestInterval;
  ...
  function rain (ctx, t, cx, cy, cw, s, color) {
    ...
    const b = TAU * 11 / 12;
    const c = TAU * 7 / 12;
    let i; let p; let x; let y;

    ctx.fillStyle = color.rain || color;

    for (i = 4; i--;) {
      p = (t + i / 4) % 1;
      x = cx + (i - 1.5) / 1.5 * (i === 1 || i === 2
        ? -1
        : 1) * a;
      y = cy + p * p * cw;
      ctx.beginPath();
  ...
  function sleet (ctx, t, cx, cy, cw, s, color) {
    ...
    for (i = 4; i--;) {
      p = (t + i / 4) % 1;
      x = Math.floor(cx + (i - 1.5) / 1.5 * (i === 1 || i === 2
        ? -1
        : 1) * a) + 0.5;
      y = cy + p * cw;
  ...
  const Skycons = function (opts) {
    ...
    this.list = [];
    this.interval = null;
    this.monochrome = typeof opts.monochrome === "undefined"
      ? true
      : opts.monochrome;
  ...
  Skycons.prototype = {
    _determineDrawingFunction (draw) {
      if (typeof draw === "string") {
        draw = Skycons[draw.toUpperCase().replace(/-/gu, "_")] || null;
      }
